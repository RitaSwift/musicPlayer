...
import com.joy.player.widget.Tintable;
...
public class ThemeUtils {
    ...
    public static Drawable tintDrawable(Context context, int resId, int colorId) {
    ...
    public static Drawable tintDrawableByDrawableId(Context context,int resId, @ColorInt int color) {
        if (resId <= 0) return null;
        Drawable drawable = context.getResources().getDrawable(resId);
        return tintDrawable(drawable, color);
    }

    public static Drawable tintDrawableByColorId(Context context, Drawable drawable, int colorId) {
        if (drawable == null) return null;
    ...
    public static Drawable tintDrawableByColorList(Context context, Drawable drawable,int colorListId, PorterDuff.Mode mode) {
        if (drawable == null) return null;
    ...
    static MusicUtils.TintInfo parseColorStateList(ColorStateList origin) {
        ...
        if (colorList.size() > 1) {
            return new MusicUtils.TintInfo(stateList, colorList);
        } else {
            return null;
        }
    }
    ...
    public static ColorStateList getThemeColorStateList(Context context, ColorStateList origin) {
        ...
        if (origin.isStateful()) {
            MusicUtils.TintInfo tintInfo = parseColorStateList(origin);
