...
import com.joy.player.util.DrawableUtils;
import com.joy.player.util.MusicUtils;
import com.joy.player.util.ThemeUtils;
import com.joy.player.util.TintManager;
...
public class AppCompatCompoundDrawableHelper extends AppCompatBaseHelper {
    ...
    private MusicUtils.TintInfo[] mCompoundDrawableTintInfos = new MusicUtils.TintInfo[4];
    ...
    public void setCompoundDrawablesTintList(int... resIds) {
        for (int i = 0; i < resIds.length; i++) {
            ...
            MusicUtils.TintInfo tintInfo = mCompoundDrawableTintInfos[i];
            if (tintInfo != null) {
    ...
    private Drawable setSupportCompoundDrawableByPosition(int position, int tintId) {
        if (tintId != 0) {
            if (mCompoundDrawableTintInfos[position] == null) {
                mCompoundDrawableTintInfos[position] = new MusicUtils.TintInfo();
            }
    ...
    private Drawable applySupportCompoundDrawableTint(int position) {
        ...
        MusicUtils.TintInfo tintInfo = mCompoundDrawableTintInfos[position];
